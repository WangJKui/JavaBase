...
public class TestChinaIDCard {

	public static void main(String[] args) {
		System.out.println(checkCard(""));
	}

	public static String checkCard(String cardId) {
		if (cardId.length() == 15 || cardId.length() == 18) {
			if (!cardCodeVerifySimple(cardId)) {
				return "15位或18位身份证号码不正确";
			} else {
				if (cardId.length() == 18 && !cardCodeVerify(cardId).equals("正确")) {
					return cardCodeVerify(cardId);
				}
				return "正确";
			}
		} else {
			return "身份证号码长度必须等于15或18位";
		}
	}
	...
	public static String cardCodeVerify(String cardcode) {
		int i = 0;
		String r = "error";
		String lastnumber = "";
